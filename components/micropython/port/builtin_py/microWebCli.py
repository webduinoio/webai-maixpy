...
class MicroWebCli :
    ...
    def OpenRequest( self,
        ...
        try:
            ...
            while 1:
                try:
                    self._socketAddr = socket.getaddrinfo(host, port, 0, socket.SOCK_STREAM)
                    self._socketAddr = self._socketAddr[0]
                    break
                except Exception:
                    err += 1
                if err > 5:
                    raise Exception("get ip failed!")
            cli = socket.socket(self._socketAddr[0], self._socketAddr[1], self._socketAddr[2])
            cli.settimeout(self.ConnTimeoutSec)
            cli.connect(self._socketAddr[-1])
        except:

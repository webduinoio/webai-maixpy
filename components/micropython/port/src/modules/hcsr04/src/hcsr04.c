...
static long hcsr04_measure_us(uint8_t trig,uint8_t echo, uint32_t timeout_us)
{
    ...
    if( fpioa_get_io_by_function(trig) < 0 )
        return -2;
    if(trig >= FUNC_GPIO0 && trig <= FUNC_GPIO7)
    {
    ...
    else if(trig >= FUNC_GPIOHS0 && trig <= FUNC_GPIOHS31)
    {
    ...
    if( fpioa_get_io_by_function(echo) < 0 )
        return -2;
    if(echo >= FUNC_GPIO0 && echo <= FUNC_GPIO7)
    {
        set_mode = gpio_set_drive_mode;
        set_pin = gpio_set_pin;
        get_pin = gpio_get_pin;
    ...
    else if(echo >= FUNC_GPIOHS0 && echo <= FUNC_GPIOHS31)
    {
        set_mode = gpiohs_set_drive_mode;
        set_pin = gpiohs_set_pin;
        get_pin = gpiohs_get_pin;
...
{
    long us = hcsr04_measure_us(trig,echo, timeout_us);
    if(us > 0)
        return us/29/2;
    return us;
}
